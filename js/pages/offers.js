// ══ OFFERS PAGE ═════════════════════════════════════════
import { stores, storeOffers, followedStores } from '../state.js';
  ...
  [...stores].sort(() => Math.random() - .5).forEach(store => {
    ...
    post.innerHTML = `
      <div class="op-header">
        ...
        <button class="op-follow-btn ${isFollowed ? 'following' : ''}"
          onclick="window._toggleFollowPost(${store.id},this)">
      ...
      <div class="op-actions">
        ...
        <button class="op-share-btn" onclick="window._sharePost('${store.name}')" title="Share">
          ${shareSVG}
        </button>
        <button class="op-link-btn"
          onclick="event.stopPropagation();window._getAmbassadorLink(${store.id})"
          title="Copy my ambassador link">🔗</button>
      </div>
...
export function sharePost(name) {
  if (navigator.share)
    navigator.share({ title: 'SelemExpress Explore', text: 'Check out ' + name + ' on SelemExpress!', url: location.href }).catch(() => {});
