// ══ EXPLORE PAGE ════════════════════════════════════════
import { stores, storeOffers, storeProducts, categories, sizes, colorSwatches } from '../data/mock.js';
import { followedStores, savedItems, exploreCat, setExploreCat, cartItems } from '../state.js';
import { toast, fmtFollowers, starsHTML, escapeAttr, getProductGallery, updateCartBadge } from '../ui.js';
...
export function renderStoreSearchGrid(list) {
  ...
  list.forEach(store => {
    ...
    card.innerHTML = `
      ...
      <button class="store-search-follow ${isFollowed ? 'following' : ''}" onclick="event.stopPropagation();window._toggleFollow(${store.id},this)">${isFollowed ? 'Following ✓' : 'Follow'}</button>`;
  ...
  document.getElementById('modal-content').innerHTML = `
    <div class="sm-header">
      ...
      <div class="sm-info">
        ...
        <div class="sm-meta"><span class="sm-followers">${fmtFollowers(store.followers)} followers</span><span>·</span><span>${store.tag}</span></div>
      ...
      <button class="sm-follow-btn ${isFollowed ? 'following' : ''}" onclick="window._toggleFollow(${store.id},this)">${isFollowed ? 'Following ✓' : 'Follow'}</button>
      <button class="sm-close-btn" onclick="document.getElementById('store-modal').classList.remove('open')">✕</button>
    </div>
    ${offer ? `<div class="sm-offer"><img src="${offer.bg}" alt="offer" loading="lazy"><div class="sm-offer-overlay"></div><div class="sm-offer-content"><span class="sm-offer-badge">${offer.badge}</span><div class="sm-offer-title">${offer.title}</div><div class="sm-offer-sub">${offer.sub}</div></div></div>` : ''}
    <div class="sm-products-label">This Week's Models</div>
    <div class="sm-products">${products.map((p, i) => `<div class="sm-prod" onclick="document.getElementById('store-modal').classList.remove('open');window._openProductPanel({id:${store.id * 100 + i},name:'${store.name} Model ${i + 1}',price:${p.price},storeId:${store.id},img:'${p.img}'})"><img src="${p.img}" alt="shoe" loading="lazy"><div class="sm-prod-price">${p.price} TND</div></div>`).join('')}</div>`;
...
export function unhoverStars(storeId) {
  document.querySelectorAll(`#sm-stars-${storeId} .star`).forEach(s => s.classList.remove('hover-lit'));
}

export function rateStore(storeId, rating) {
  stores.find(s => s.id === storeId).rating = rating;
  document.querySelectorAll(`#sm-stars-${storeId} .star`).forEach(s => {
    const v = parseInt(s.dataset.val);
...
export function openProductPanel(shoe) {
  ...
  document.getElementById('prod-content').innerHTML = `
    <div class="pd-gallery-wrap">
      <button class="pd-close-btn" onclick="document.getElementById('prod-panel').classList.remove('open')" aria-label="Close product">✕ <span>Close</span></button>
      <div class="pd-gallery" id="pd-gallery">
        ${gallery.map((img, idx) => `<div class="pd-gallery-slide"><img class="pd-img" src="${img}" alt="${shoe.name} photo ${idx + 1}" loading="${idx === 0 ? 'eager' : 'lazy'}"></div>`).join('')}
      </div>
      ${gallery.length > 1 ? `<button class="pd-gallery-btn prev" onclick="window._moveProductGallery(-1)">‹</button><button class="pd-gallery-btn next" onclick="window._moveProductGallery(1)">›</button><div class="pd-gallery-dots" id="pd-gallery-dots">${gallery.map((_, i) => `<span class="pd-gallery-dot ${i === 0 ? 'active' : ''}"></span>`).join('')}</div>` : ''}
    </div>
    <div class="pd-body">
      <div class="pd-store-row" onclick="document.getElementById('prod-panel').classList.remove('open');window._openStoreModal(window._stores.find(s=>s.id===${store.id}))">
      ...
      <div class="pd-option-group">
        ...
        <div class="pd-colors" id="pd-colors">
          ${colorSwatches.map((c, i) => `<div class="color-swatch ${i === 0 ? 'selected' : ''}" style="background:${c};${c === '#ffffff' ? 'border:1.5px solid #ddd;' : ''}" onclick="window._selectColor(this,'${c}')" title="${c}"></div>`).join('')}
        </div>
      </div>
      <div class="pd-option-group">
        <div class="pd-sizes-label">Size (EU)</div>
        <div class="pd-sizes">${sizes.map((s, i) => `<button class="size-btn ${i === 4 ? 'selected' : ''}" onclick="window._selectSize(this)">${s}</button>`).join('')}</div>
      </div>
      <div class="pd-actions">
        <button class="pd-cart-btn" onclick="window._addToCartFromPanel(${shoe.id},'${escapeAttr(shoe.name || 'Shoe')}',${shoe.price},'${shoe.img}','${escapeAttr(store.name)}',${store.id})">Add to Cart 🛒</button>
        <button class="pd-save-btn" id="pd-save-${shoe.id}" onclick="window._toggleSave(${shoe.id},this)">${isSaved ? '❤️' : '🤍'}</button>
      </div>
    </div>`;
...
export function handleSearch(q) {
  ...
  const results = qLow ? stores.filter(s => s.name.toLowerCase().includes(qLow) || s.tag.toLowerCase().includes(qLow)) : stores;
