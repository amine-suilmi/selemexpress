// ══ CART PAGE ════════════════════════════════════════════
import { cartItems, savedItems, savedAddress, addressModalSource, editingCartIndex, setAddressModalSource, setEditingCartIndex, setCartItems } from '../state.js';
import { toast, getColorName, updateCartBadge } from '../ui.js';
import { colorSwatches, sizes } from '../data/mock.js';

export function renderCart() {
  const wrap = document.getElementById('cart-wrap');
  if (!cartItems.length) {
    wrap.innerHTML = `<div class="cart-empty"><div class="big-ic">🛒</div><h3>Your cart is empty</h3><p>Browse our feed and add shoes you love.</p></div>`;
    return;
  }
  let html = '';
  cartItems.forEach((item, idx) => {
    html += `<div class="cart-item">
      ...
      <div class="cart-item-info">
        ...
        <div class="cart-item-meta-row">
          <span class="cart-color-chip"><span class="cart-color-dot" style="background:${item.color}"></span>${getColorName(item.color)}</span>
  ...
  html += `<div class="cart-footer">
    ...
    <div class="address-note">${savedAddress ? `Deliver to: ${savedAddress}` : 'No address saved yet. Add one from the button above or from Settings.'}</div>
  </div>`;
  wrap.innerHTML = html;
}
...
export function checkout() {
  toast(savedAddress ? 'Order placed! 🎉 Delivery in 2-3 days' : 'Order placed! 🎉 You can still add an address from cart settings');
  cartItems.length = 0;
  updateCartBadge(cartItems);
  renderCart();
}
...
export function addToCartFromPanel(id, name, price, img, storeName, storeId) {
  ...
  const color = colEl ? colEl.getAttribute('title') || colEl.style.background : 'black';
  const size = sel.textContent;
  const existing = cartItems.find(i => i.id === id && i.size === size && i.color === color && String(i.storeId) === String(storeId));
  if (existing) { existing.qty++; }
  else { cartItems.push({ id, name, price, img, storeName, storeId, size, color, qty: 1 }); }
  updateCartBadge(cartItems);
  toast('Added to cart 🛒');
...
export function openCartEdit(idx) {
  ...
  document.getElementById('cart-edit-content').innerHTML = `
    ...
    <div class="pd-option-group">
      ...
      <div class="pd-colors">${colorSwatches.map(c => `<div class="color-swatch ${c === item.color ? 'selected' : ''}" style="background:${c};${c === '#ffffff' ? 'border:1.5px solid #ddd;' : ''}" onclick="window._selectCartEditColor(this,'${c}')" title="${c}"></div>`).join('')}</div>
    </div>
    <div class="pd-option-group" style="margin-top:14px;">
      <div class="pd-sizes-label">Size (EU)</div>
      <div class="pd-sizes">${sizes.map(s => `<button class="size-btn ${String(s) === String(item.size) ? 'selected' : ''}" onclick="window._selectCartEditSize(this)">${s}</button>`).join('')}</div>
    </div>
...
export function saveCartEdit() {
  // read editingCartIndex from state
  ...
  const duplicateIndex = cartItems.findIndex((it, i) => i !== idx && it.id === item.id && String(it.storeId) === String(item.storeId) && String(it.size) === String(nextSize) && String(it.color) === String(nextColor));
  if (duplicateIndex > -1) {
    cartItems[duplicateIndex].qty += item.qty;
    cartItems.splice(idx, 1);
  } else {
    item.size = nextSize;
